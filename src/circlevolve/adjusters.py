...
DARK_ADJUST_PARAMETERS = [17.1976, 256, 512, -262.665]


def dark_adjust(original_image):
    global DARK_ADJUST_PARAMETERS

    adjusted_image = original_image.copy()
    adjusted_image = adjusted_image + DARK_ADJUST_PARAMETERS[0]
    adjusted_image = np.log(adjusted_image) / np.log(DARK_ADJUST_PARAMETERS[1])
    adjusted_image = adjusted_image * DARK_ADJUST_PARAMETERS[2]
    adjusted_image = adjusted_image + DARK_ADJUST_PARAMETERS[3]
...
def dark_unadjust(adjusted_image):
    ...
    original_image = adjusted_image.copy()
    original_image = original_image - DARK_ADJUST_PARAMETERS[3]
    original_image = original_image / DARK_ADJUST_PARAMETERS[2]
    original_image = np.exp2(log2(DARK_ADJUST_PARAMETERS[1]) * original_image)
    original_image = original_image - DARK_ADJUST_PARAMETERS[0]
